...
async function checkActivityType(req, res, next) {
    try {
        ...
        // Use comprehensive activity type validation
        if (!activityTypes.isValidActivityType(activity.type)) {
            log.warn('Invalid activity type received', null, {
                receivedType: activity.type,
                validTypes: Object.values(activityTypes.ACTIVITY_TYPES),
                actor: activity.actor,
                activityId: activity.id,
                platform: activityTypes.detectPlatform(req.get('User-Agent'), req.get('Host'))
            });
            return res.status(400).json({
        ...
        // Validate the complete activity structure
        try {
            activityTypes.validateActivity(activity);
        } catch (validationError) {
            log.warn('Activity structure validation failed', null, {
            ...
            return res.status(400).json({
                error: 'Bad Request: Invalid activity structure',
