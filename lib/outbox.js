...
async function getOutbox(req, res) {
	try {
		const collection = client.db(database).collection("create");
...
async function postOutbox(req, res) {
    try {
        ...
        const activityId = `https://www.sneaas.no/u/trondss/notes/${uuid}`;
        // New create activities are saved to the create-collection
        ...
        // Notes are saved to create-collection. Likes and announces are saved to their respective collections (like and announce)
