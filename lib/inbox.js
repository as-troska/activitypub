...
async function postInbox(req, res) {
    try {
        ...
        if (activity.type === 'Follow') {
            ...
            const followerExists = await followersCollection.findOne({actor: activity.actor});
            ...
            if (!followerExists) {
                await followersCollection.insertOne({actor: activity.actor}); // Save only the actor
                await followActivitiesCollection.insertOne(activity); // Save the Follow activity
            }
