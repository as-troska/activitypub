...
async function postInbox(req, res) {
    try {
        ...
        const announcesCollection = client.db(database).collection("announces");
        const acceptCollection = client.db(database).collection("accepts");
        ...
        switch (activity.type) {
            ...
            case 'Create':
                if (activity.object.type === 'Note') {
                    if (activity.object.inReplyTo) {
                        ...
                        const originalNote = await notesCollection.findOne({id: originalNoteId});
            ...
            case 'Like':
                ...
                if (!like) {
                    ...
                    await outboxCollection.updateOne({id: activity.object}, {$inc: {'object.likes': 1}});
                } else {
            ...
            case 'Announce':
                ...
                if (!announce) {
                    ...
                    await outbox.updateOne({"object.id": activity.object}, {$inc: {'object.shares': 1}});
                } else {
