            ...
            case 'Create':
                if (activity.object.type === 'Note') {
                    if (activity.object.inReplyTo) {
                        // The following logic is for handling replies/comments. Seems to be working for now.
                        const originalNoteId = activity.object.inReplyTo;
                        const originalNote = await createCollection.findOne({id: originalNoteId});
            
                        if (originalNote) {
                            await createCollection.updateOne({id: originalNoteId}, {$inc: {replies: 1}});
                            await createCollection.updateOne({id: originalNoteId}, {$inc: {'object.replies': 1}});
                        } else {
                            console.log(`Original note ${originalNoteId} not found`);
                        }            
                        await commentsCollection.insertOne(activity.object);
                    } else {
                        // This is what will happen with followed users notes broadcasted to my inbox. Might need to change this...
